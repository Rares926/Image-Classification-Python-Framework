...
class IOHelper:
    ...
    def check_if_dir_exists(dir_path: str, custom_error_message: bool = None) -> None:
        if not IOHelper.dir_exists(dir_path):
            error_message = 'The specified dir path does not exist' if custom_error_message == None else custom_error_message
    ...
    def check_if_file_exists(file_path: str, custom_error_message: bool = None) -> None:
        if not IOHelper.file_exists(file_path):
            error_message = 'The specified file path does not exist' if custom_error_message == None else custom_error_message
    ...
    @staticmethod
    def get_files(dir_path: str, extensions: Tuple[str] , full_path: bool = False, recursively: bool = False) -> List[str]:
        if full_path:
            files = [os.path.join(dir_path, f) for f in os.listdir(dir_path) if os.path.isfile(os.path.join(dir_path, f)) and f.lower().endswith(extensions)]
        else:
